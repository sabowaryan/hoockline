...
import { Home, RefreshCw, AlertTriangle, Server, Wifi, WifiOff } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20 mb-6">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <h3 className="text-xl font-bold text-gray-900">
              {t('error.500.troubleshootingTitle')}
            </h3>
          </div>
          ...
          <div className="space-y-3 text-left">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">{t('error.500.checkConnection')}</p>
                <p className="text-xs text-gray-500">{t('error.500.checkConnectionDesc')}</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">{t('error.500.clearCache')}</p>
                <p className="text-xs text-gray-500">{t('error.500.clearCacheDesc')}</p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">{t('error.500.tryLater')}</p>
                <p className="text-xs text-gray-500">{t('error.500.tryLaterDesc')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-2xl p-6 border border-blue-200">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <h3 className="text-xl font-bold text-gray-900">
              {t('error.500.tryGeneratorTitle')}
            </h3>
          </div>
          <p className="text-gray-600 mb-4">
            {t('error.500.tryGeneratorDescription')}
          </p>
          <button
          ...
          >
            ...
            <span>{t('error.500.tryGenerator')}</span>
          </button>
        </div>

        {/* Contact Support */}
        <div className="mt-8 text-sm text-gray-500">
          <p className="mb-2">{t('error.500.needHelp')}</p>
          <div className="flex flex-wrap justify-center gap-4">
            <button
              onClick={() => navigate('/')}
              className="text-purple-600 hover:text-purple-700 font-medium hover:underline"
            >
              {t('error.500.homePage')}
            </button>
            <button
              onClick={() => navigate('/generator')}
              className="text-purple-600 hover:text-purple-700 font-medium hover:underline"
            >
              {t('error.500.generator')}
            </button>
            <a
              href="mailto:support@clicklone.com"
              className="text-purple-600 hover:text-purple-700 font-medium hover:underline"
            >
              {t('error.500.contactSupport')}
            </a>
          </div>
        </div>
      </div>
